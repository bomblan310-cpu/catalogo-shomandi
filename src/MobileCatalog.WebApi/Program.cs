...
{
    options.SwaggerDoc("v1", new() { Title = "Shomandi Mobile Catalog API", Version = "v1" });
    options.AddSecurityDefinition("AdminApiKey", new()
    {
        Type = Microsoft.OpenApi.Models.SecuritySchemeType.ApiKey,
        Name = "X-Admin-Key",
        In = Microsoft.OpenApi.Models.ParameterLocation.Header,
        Description = "Clave requerida por los endpoints administrativos."
    });
    options.AddSecurityRequirement(new()
    {
        [new Microsoft.OpenApi.Models.OpenApiSecurityScheme
        {
            Reference = new() { Type = Microsoft.OpenApi.Models.ReferenceType.SecurityScheme, Id = "AdminApiKey" }
        }] = []
    });
});
...
builder.Services.AddSingleton(new WhatsAppLinkService(builder.Configuration["WhatsApp:BaseUrl"] ?? "https://wa.me"));
...
{
    options.SwaggerEndpoint("/swagger/v1/swagger.json", "Shomandi API v1");
    options.DocumentTitle = "Shomandi API";
});
